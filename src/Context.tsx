...
const AppProvider: React.FC <Props> = ({ children }) => {
    ...
    try{
        const res = await axios.get('backendsss.azurewebsites.net/films')
...
const addFilm = async(Title : string, Description: string) => {
  try {
    const response = await axios.post('backendsss.azurewebsites.net/films', {
      title: Title,
...
const deleteFilm = async (filmId: number):Promise<void> => {
  try {
    await axios.delete(`backendsss.azurewebsites.net/films/${filmId}`)
...
const updateFilm = async(filmId: number, Title : string, Description: string) => { 
  try {
    const response = await axios.put('backendsss.azurewebsites.net/films', {
      title: Title,
      description: Description,
      film_id : filmId
    })
